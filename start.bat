...
REM Case C: pip broken (same root cause — base Python path changed)
"%VENV_PIP%" --version >nul 2>&1
if errorlevel 1 goto :rebuild_venv

REM Case D: python & pip OK but packages not installed
...
:rebuild_venv
echo   [!!] .venv is broken (base Python path changed) — rebuilding...
rmdir /s /q "%VENV_DIR%"
